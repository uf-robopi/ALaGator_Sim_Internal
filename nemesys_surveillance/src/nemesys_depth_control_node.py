...
class DepthControlNode:
    def __init__(self):
        rospy.init_node('nemesys_depth_control_node')
 
        # Subscribers
        self.depth_sub = rospy.Subscriber(
            name="/nemesys/raw_depth_data", data_class=LaserScan, callback=self.depth_callback)
        self.depth_sub = rospy.Subscriber(
            name="/nemesys/target_depth", data_class=Float32, callback=self.target_depth_callback)
        self.depth_change_indicator_sub = rospy.Subscriber(
            name='/nemesys/depth_change_indicator', data_class=Bool, callback=self.adjust_target_depth_callback)
       
        # Publisher
        self.depth_control_pub = rospy.Publisher(
            name="/nemesys/heave_control_input", data_class=Float32, queue_size=1)
