...
class DepthControlNode:
    def __init__(self):
        rospy.init_node('cavepi_depth_control_node')
 
        # Subscribers
        self.depth_sub = rospy.Subscriber(
            name="/cavepi/raw_depth_data", data_class=LaserScan, callback=self.depth_callback)
        self.depth_sub = rospy.Subscriber(
            name="/cavepi/target_depth", data_class=Float32, callback=self.target_depth_callback)
        self.depth_change_indicator_sub = rospy.Subscriber(
            name='/cavepi/depth_change_indicator', data_class=Bool, callback=self.adjust_target_depth_callback)
       
        # Publisher
        self.depth_control_pub = rospy.Publisher(
            name="/cavepi/heave_control_input", data_class=Float32, queue_size=1)
